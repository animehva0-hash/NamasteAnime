...
async function Content() {
  try {
    if (await needsSync("popular", 6)) {
      await syncCategory({ category: "popular", query: `query($page:Int,$perPage:Int){Page(page:$page,perPage:$perPage){media(type:ANIME,sort:POPULARITY_DESC){${MF}}}}`, variables: { page: 1, perPage: 24 } }).catch(() => {});
    }
    const data = await getFromCache("popular");
    return (
      <div>
        <h1 className="text-2xl sm:text-3xl font-bold text-white mb-2">Popular Anime</h1>
