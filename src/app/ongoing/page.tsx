...
async function Content() {
  try {
    if (await needsSync("ongoing", 6)) {
      await syncCategory({ category: "ongoing", query: `query($page:Int,$perPage:Int){Page(page:$page,perPage:$perPage){media(type:ANIME,status:RELEASING,sort:POPULARITY_DESC){${MF}}}}`, variables: { page: 1, perPage: 24 } }).catch(() => {});
    }
    const data = await getFromCache("ongoing");
    return (
      <div>
        <h1 className="text-2xl sm:text-3xl font-bold text-white mb-2">Currently Airing</h1>
        <p className="text-text-secondary text-sm mb-6">Ongoing anime</p>
