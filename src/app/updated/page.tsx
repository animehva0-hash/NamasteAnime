...
async function Content() {
  try {
    if (await needsSync("updated", 6)) {
      await syncCategory({ category: "updated", query: `query($page:Int,$perPage:Int){Page(page:$page,perPage:$perPage){media(type:ANIME,sort:UPDATED_AT_DESC,status:RELEASING){${MF}}}}`, variables: { page: 1, perPage: 24 } }).catch(() => {});
    }
    const data = await getFromCache("updated");
    return (
      <div>
        <h1 className="text-2xl sm:text-3xl font-bold text-white mb-2">Recently Updated</h1>
